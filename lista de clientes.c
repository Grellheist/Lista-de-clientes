...
Dados *inicia_dados(char *nome, int idade, int cpf);
Dados *insere_dados(Dados *dados, char *nome, int idade, int cpf);
...
/* apenas checa se a lista é NULL ou não. */
int checa_vazio(Dados *dados) {
...
void insere(void) {
        ...
        if (principal == NULL)
                principal = inicia_dados(nome, idade, cpf);
        else
                principal = insere_dados(principal, nome, idade, cpf);
}
